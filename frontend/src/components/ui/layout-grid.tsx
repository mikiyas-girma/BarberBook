...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    <div className=" h-full p-10 grid grid-cols-1 md:grid-cols-3  max-w-7xl mx-auto gap-y-4 relative">
      {cards.map((card, i) => (
...
const SelectedCard = ({ selected }: { selected: Card | null }) => {
  return (
    <div className=" h-full flex flex-col justify-end rounded-lg  z-[60]">
      <motion.div
        initial={{
          opacity: 0,
        }}
        ...
        className="absolute  h-full bg-black opacity-60 z-10"
      />
      <motion.div
